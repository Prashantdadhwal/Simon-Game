...
// Detect keypress and map to colors
$(document).keypress(function(event) {
  const pressedKey = event.key; // Get the key pressed
  if (keyColorMap[pressedKey]) {
    const userChosenColour = keyColorMap[pressedKey]; // Map key to color
    userClickedPattern.push(userChosenColour); // Add color to user pattern
    playSound(userChosenColour); // Play sound for the color
    animatePress(userChosenColour); // Animate button press
    checkAnswer(userClickedPattern.length - 1); // Check the answer
  }
});

// Add to game.js (inside document ready)
$(document).ready(function() {
  $(".help-icon").click(function(e) {
    e.stopPropagation();
    $(".help-message").toggleClass("active");
  });

  $(document).click(function(e) {
    if (!$(e.target).closest('.help-icon, .help-message').length) {
      $(".help-message").removeClass("active");
    }
  });
});
